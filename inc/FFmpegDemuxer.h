...
public:
    explicit FFmpegDemuxer(const char* szFilePath, const std::map<std::string, std::string>& ffmpeg_options);
    explicit FFmpegDemuxer(DataProvider& pDataProvider, const std::map<std::string, std::string>& ffmpeg_options);
    ~FFmpegDemuxer();
    ...
    bool Demux(uint8_t*&   pVideo,
               size_t&     rVideoBytes,
               PacketData& pktData,
               uint8_t**   ppSEI     = nullptr,
               size_t*     pSEIBytes = nullptr);

    bool Seek(SeekContext& seek_ctx,
              uint8_t*&    pVideo,
              size_t&      rVideoBytes,
              PacketData&  pktData,
              uint8_t**    ppSEI     = nullptr,
...
inline int FFmpeg2IxCodecId(AVCodecID id)
{
    switch (id)
    {
